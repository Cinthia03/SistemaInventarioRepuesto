...
export type { Rubro };
export type RubroAcabado = Rubro;
...
export class RubrosObraAcabadosService {
  ...
  private readonly NOMBRE_CATEGORIA = 'ACABADOS';

  constructor(private supabaseService: SupabaseService) {}

  private getCategoriaAcabadosId(): Observable<number> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) {
          console.error('Error al obtener categoría ACABADOS:', error);
          throw error;
        }
        return data.id;
      })
  ...
  private getSubcategoriasConId(): Observable<{ id: number; nombre: string }[]> {
    return this.getCategoriaAcabadosId().pipe(
      ...
      map(({ data, error }: any) => {
        if (error) {
          console.error('Error al cargar subcategorías de Acabados:', error);
          throw error;
        }
        return data || [];
      })
  ...
  getSubcategoriasAcabados(): Observable<string[]> {
    return this.getSubcategoriasConId().pipe(
      map(subs => subs.map(s => s.nombre))
    );
  }

  getRubrosAcabados(): Observable<Rubro[]> {
    return this.getSubcategoriasConId().pipe(
      switchMap(subs => {
        if (subs.length === 0) {
          return of([] as any[]);
        ...
        return from(
          ...
          map(({ data, error }: any) => {
            if (error) {
              console.error('Error al consultar acabados_rubros:', error);
              throw error;
            }
            return (data || []).map((item: any) => ({
              ...item,
              apu_detalles: item.acabados_apu_detalles,
  ...
  getRubros(): Observable<Rubro[]> {
    return this.getRubrosAcabados();
  }

  getCategorias(): Observable<string[]> {
    return this.getSubcategoriasAcabados();
  }
}
